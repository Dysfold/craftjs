import { Class, Object, Runnable } from "java.lang";
...
function getSpigotMethod(name: 'runTaskTimer' | 'runTaskLater') {
  ...
  const c = (server.getScheduler() as any).class as Class<BukkitScheduler & Object>;
  const params = [(Plugin as any).class, (Runnable as any).class, long].concat(name === 'runTaskTimer' ? [long] : []);
  console.log(params);
...
function setInterval(handler: () => void, delay: number) {
  ...
  const task = runTaskLater(__plugin, runnable, delay / 50, delay / 50) as BukkitTask;
